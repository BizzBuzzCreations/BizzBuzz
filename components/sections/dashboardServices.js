...
function ListField({ field, items, onChange }) {
  ...
  // uploaded inside a list item only updated local state and silently
  // reverted if the admin didn't remember to click "Save Changes" after.
...
// Every one of the 8 main service hub pages (BPO, Website Development,
// SEO, Social Media Marketing, Google Ads & Paid Marketing, AI Solutions,
// Marketing Automation, Business Consultancy) shares the exact same
// structure, so instead of 8 separate dashboard entries, this is one
// picker: choose the service, then edit that page's hero, intro, services
// grid, roadmap, we-are section, FAQ, trust section, and closing CTA —
// same section shapes for every one of the 8 pages.
export default function DashboardServices() {
  const [pageKey, setPageKey] = useState(SERVICE_CONTENT_REGISTRY[0]?.key || "");
  const [values, setValues] = useState({});
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [savedAt, setSavedAt] = useState(null);

  // getPageMeta (not a raw find on SERVICE_CONTENT_REGISTRY) so this
  // page picks up the shared SEO/Meta section auto-appended to every page.
  ...
  return (
    <section className="mt-6 rounded-2xl border border-slate-100 bg-white p-6">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-base font-bold text-slate-900">Services Pages</h2>
          <p className="text-xs text-slate-400">
            The 8 main service pages share the same layout — pick one below
            to edit its text and images. Design and layout stay exactly as
            built.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            >
              {SERVICE_CONTENT_REGISTRY.map((p) => (
                <option key={p.key} value={p.key}>
                  {p.label.replace(/^Services — /, "")}
                </option>
              ))}
            </select>
            <svg
