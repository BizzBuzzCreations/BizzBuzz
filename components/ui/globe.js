...
// Globe — adapted from Originkit's Framer component (fetched via
// `npx originkit add globe`) for plain React/Next.js. Framer-only bits
// removed: `addPropertyControls`/`ControlType` (Framer's Canvas property
// panel — meaningless outside Framer), `RenderTarget.current()` (Framer
// Canvas-vs-Preview detection), the `ComponentMessage` CDN import (swapped
// for a plain inline error state), and the `@framerSupportedLayoutWidth`
// style JSDoc annotations. The Three.js/d3-geo globe logic itself —
// dot-mapped world map from a live GeoJSON land dataset, drag-to-rotate
// with momentum, auto-rotation, markers — is unchanged.
...
// CSS variable token and color parsing (hex/rgba/var()) — harmless no-op
// for plain hex/rgba colors, just here so any var(--token, #fallback)
// passed in still resolves sensibly.
...
// All UI sliders in the original Framer panel were whole numbers; these
// map that "UI scale" to the internal values the renderer actually uses.
...
// Convert lat/lng to a position on the unit sphere (Three.js: Y up, Z
// forward, X right).
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (event) => {
      ...
      const handleMouseMoveDrag = (moveEvent) => {
        ...
        const dxMove = moveEvent.clientX - lastMouseX;
        const dyMove = moveEvent.clientY - lastMouseY;
        targetRotation.x += dxMove * sensitivity;
        targetRotation.y += dyMove * sensitivity;
        targetRotation.y = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, targetRotation.y));
        velocity.x = dxMove * sensitivity * 0.3;
        velocity.y = dyMove * sensitivity * 0.3;
